...
platelet_diameter = lambda g, b: (2 * g + 1) * b   # diameter of platelet with g generations (2*G + 1 center)
...
def pack_flakes_layered(flakes: List[np.ndarray], phi: float, b_len: float, max_platelet_diameter: float, max_restarts=10):
    ...
    z_spacing = 1.06  # Fixed z-spacing between layers

    # Generate z-layers
    n_layers = int(L / z_spacing)
    if n_layers % 2 == 0:
        n_layers -= 1
    z_layers = np.linspace(-L/2, L/2, n_layers)
    print(f"[info] Using {len(z_layers)} z-layers with {z_spacing}σ spacing")
    ...
    for restart in range(max_restarts):
        ...
        # Go through each z-layer
        for layer_idx, z in enumerate(z_layers):
            if flake_idx >= len(flakes):
                break
            
            print(f"[info] Layer {layer_idx + 1}/{len(z_layers)} at z={z:.3f}σ")
            layer_placed = 0
            
            # Try to place as many flakes as possible in this xy-plane
            while flake_idx < len(flakes):
                flake = flakes[flake_idx]
                placed = False
                ...
                for _ in range(10000):  # Reduced attempts per flake in layer
                    ...
                    # Check if flake fits in box
                    if (np.all(trial >= -L/2) and np.all(trial <= L/2) and
                        (len(kd.data) == 0 or kd.query(trial, k=1)[0].min() >= 1.05 * b_len)):
                ...
                if not placed:
                    # Move to next layer if can't place more in this one
                    print(f"[info]   Layer {layer_idx + 1} filled with {layer_placed} flakes")
                    break
            
            if layer_placed == 0 and flake_idx < len(flakes):
                print(f"[info] Could not place any flakes in layer {layer_idx + 1}")

        if flake_idx == len(flakes):
            actual_phi = total_beads * bead_vol / (L ** 3)
            print(f"[info] packing successful: φ_actual = {actual_phi:.3f}")
            print(f"[info] Placed all {len(flakes)} flakes across {len(z_layers)} layers")
            return np.asarray(centres), bonds, L, flake_sizes
        else:
            print(f"[info] Only placed {flake_idx}/{len(flakes)} flakes")
            success = False
...
if __name__ == "__main__":
    ...
    # ---------------- pack + write -----------------------------------------
    centres, bonds, L, flake_sizes = pack_flakes_layered(flakes, P.phi, P.b, max_diameter)
