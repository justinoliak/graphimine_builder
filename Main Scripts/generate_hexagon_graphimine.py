...
def read_xyz(path): with open(path) as f: _ = f.readline(); _ = f.readline(); lines = f.readlines(); return [line.split()[0] for line in lines], np.array([[float(x) for x in line.split()[1:4]] for line in lines])
def write_xyz(path, elems, coords, comment=""): with open(path,'w') as f: f.write(f"{len(elems)}\n{comment}\n"); [f.write(f"{e} {x:10.6f} {y:10.6f} {z:10.6f}\n") for e, (x,y,z) in zip(elems, coords)]
def load_monomer(path): elems, coords = read_xyz(path); return [(e, *(pt-coords.mean(axis=0))) for e,pt in zip(elems, coords)]
def generate_hex_centers(N, a): ctrs = []; [ctrs.append((a*(i+0.5*j), a*(math.sqrt(3)/2*j), 0.0)) for i in range(-N, N+1) for j in range(max(-N, -i-N), min(N, -i+N)+1)]; return ctrs
def build_heavy(template, centers): elems, coords = [], []; [[[elems.append(e), coords.append([cx+dx, cy+dy, cz+dz])] for e, dx, dy, dz in template] for cx,cy,cz in centers]; return elems, np.array(coords)
...
if __name__ == "__main__":
    ...
    centers = generate_hex_centers(args.N_target, a); elems, coords = build_heavy(template, centers); elems2, coords2, imine_count, aldehyde_count = cap_edges(elems, coords)
    filename = f"N={args.N_target}_graphimine_hexagon.xyz"; write_xyz(filename, elems2, coords2, f"N_target={args.N_target} N_hex={N_hex} scale={args.scale} hexagon")
    write_counts_table(filename, args.N_target, imine_count, aldehyde_count, "hexagon"); print(f"N_target={args.N_target}, N_hex={N_hex}, scale={args.scale}, atoms={len(elems2)}, centers={len(centers)}"); print(f"Functional groups: {imine_count} imine, {aldehyde_count} aldehyde, {imine_count + aldehyde_count} total")