...
def read_xyz(path):
    with open(path) as f: 
        _ = f.readline(); _ = f.readline(); lines = f.readlines()
...
def write_counts_table(filename, n_target, amide_count, aldehyde_count, script_type): 
    import os; write_header = not os.path.exists("functional_group_counts.txt")
    with open("functional_group_counts.txt", 'a') as f:
        f.write("Filename\tScript_Type\tN_Target\tAmide_Count\tAldehyde_Count\tTotal\n" if write_header else "")
        f.write(f"{filename}\t{script_type}\t{n_target}\t{amide_count}\t{aldehyde_count}\t{amide_count + aldehyde_count}\n")
if __name__ == "__main__":
    ...
    N_hex, template, a = math.ceil(args.N_target / args.scale), load_monomer("monomer_graphamid.xyz"), C_C_RING + C_C_SINGLE + C_N_AMIDE + N_C_SINGLE + C_C_RING
    centers = [(x,y,z) for x,y,z in generate_hex_centers(N_hex, a) if x*x+y*y <= (a*args.N_target)**2]; elems, coords = build_heavy(template, centers); elems2, coords2, amide_count, aldehyde_count = cap_edges(elems, coords)
    filename = f"N={args.N_target}_graphamid.xyz"; write_xyz(filename, elems2, coords2, f"N_target={args.N_target} N_hex={N_hex} scale={args.scale}")
    write_counts_table(filename, args.N_target, amide_count, aldehyde_count, "circular"); print(f"N_target={args.N_target}, N_hex={N_hex}, scale={args.scale}, atoms={len(elems2)}, centers={len(centers)}"); print(f"Functional groups: {amide_count} amide, {aldehyde_count} aldehyde, {amide_count + aldehyde_count} total")