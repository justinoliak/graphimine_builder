#!/usr/bin/env python3
"""
Graphimine count-only script - generates CSV data for N100-N200000 with 10 runs each
"""
...
def generate_single_structure(N):
    """Generate a single structure and return counts."""
    G, _ = compute_closest_G_circular(N)
    ...
    # Find origin center
    origin_idx, _ = find_origin_center(centres)
    
    # Build adjacency list
    adjacency = build_adjacency_list(centres)
    
    # Build constrained structure
    selected_monomers = build_constrained_structure(origin_idx, adjacency, centres, N)
    ...
    return len(selected_monomers), n_imine, n_ald

def main():
    # Generate CSV data
    with open('graphimine_data.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['N_target', 'monomers', 'imines', 'aldehydes'])
        
        # N from 100 to 200000 in intervals of 100, with 10 runs each
    ...
    print("CSV file 'graphimine_data.csv' generated successfully!")
