...
def write_counts_table(filename, n_target, imine_count, aldehyde_count, script_type):
    ...
    with open("functional_group_counts.txt", 'a') as f:
        if write_header:
            f.write("Filename\tScript_Type\tN_Target\tImine_Count\tAldehyde_Count\tTotal\n")
        f.write(f"{filename}\t{script_type}\t{n_target}\t{imine_count}\t{aldehyde_count}\t{imine_count + aldehyde_count}\n")
...
if __name__ == "__main__":
    ...
    write_counts_table(filename, args.G_target, imine_count, aldehyde_count, "hexagon")
